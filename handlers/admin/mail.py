...
from keyboards.inline.user.mail import (
...
from keyboards.inline.user import get_material_format_list_inline_keyboard

from utils.admin.mail import mail_create_send_message_detail
from utils.db_api.admin.mail import get_message_template, save_message_template, delete_message_template

from states.admin.mail import MailState

from tasks.celery import task_mail
...
async def mail_material_format_list_choose(callback: types.CallbackQuery):
    material_format_list_inline_keyboard = await get_material_format_list_inline_keyboard()
...
async def mail_create__users(callback: types.CallbackQuery):
    ...
    await dp.storage.set_data(user=user_id, data={'material_format_id': 'users'})
...
async def mail_create__material_format(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    material_format_id = callback.data.replace('mail_create ', '')

    await dp.storage.set_data(user=user_id, data={'material_format_id': material_format_id})
    await mail_create_send_message_detail(callback=callback)
...
async def mail_create__message_template_choose(callback: types.CallbackQuery):
    ...
    await dp.storage.update_data(user=user_id, data=message_template.data)
    await mail_create_send_message_detail(callback=callback)
...
async def mail_create__title(callback: types.CallbackQuery):
    ...
    if callback.message.content_type in ['photo', 'video']:
        await callback.message.edit_caption(_('Отправьте заголовок'), reply_markup=mail_create_data_cancel_inline_keyboard)
    else:
...
async def mail_send(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.answer(_('Рассылка успешно запущена'))
    ...
    if time_to_mail:
        ...
        task_mail.apply_async(({'mail_data': mail_data},), eta=eta)
    else:
        # print(mail_data)
        await mail(mail_data)
        # task_mail.delay({'mail_data': mail_data})
